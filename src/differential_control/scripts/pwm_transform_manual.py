#!/usr/bin/env python
'''
This package is meant to be used with the teleop packages (teleop_twist_keyboard, etc.) for controlling
differential drive robots movements.
...
class pwm_converter():
    ...
    def twist_info_callback(self,data):
        ...
        self.motor1 = 100*(data.linear.x+(data.angular.z*self._wheelbase*self._angular_multiplier)/2)/self._default_speed
        self.motor2 = 100*(data.linear.x-(data.angular.z*self._wheelbase*self._angular_multiplier)/2)/self._default_speed
